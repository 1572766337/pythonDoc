...
class Application(Frame):
    ...
    def __init__(self, master):  # master=None
        ...
        # 初始化配置文件
        for p in sys.path:
            if p.endswith(os.sep+'lib'):
                self.path = p
                break
        if not self.path:
            fini = open(sys.path[0]+os.sep+'Py3LibPath.ini', 'a+')
            fini.seek(0)
            self.path = fini.read()
            self.path = filedialog.askdirectory(title='请选择 Python Lib 文件夹')
    ...
    def showMethodInfo(self, event):
        ...
        for idx in self.select:
            ...
            try:
                ...
                exec('self.methodInfo = pydoc.render_doc("%s")' % method)
                self.methodInfo = re.sub('\x08.', '', self.methodInfo)
                self.text.delete(0.0, END)
                self.text.insert(1.0, self.methodInfo)
            except ImportError:
                self.text.delete(0.0, END)
        ...
        for idx in self.select:
            try:
                ...
                if '.' in self.mn:
                    ...
                    self.methods = dir(__builtins__)
                else:
                    exec('import %s' % self.mn)
                    exec('self.methods = dir(%s)' % self.mn)
...
root = Tk()
root.title('PythonDoc3 By:星晨之域 QQ:1572766337')
